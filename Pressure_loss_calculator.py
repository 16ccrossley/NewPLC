...
def calculate_pressure_loss(eta0, b, T_ref, nozzle_diameter, injection_speed, shot_weight, gate_diameter, nozzle_length, mold_temp, cycle_time):
    # Calculate viscosity at the injection temperature
    T_injection = T_ref + (injection_speed * 0.1)  # Example adjustment for injection speed
...
injection_speed = st.number_input("Injection Speed (mm/s)", value=50.0, step=1.0)
...
if st.button("Calculate Pressure Loss"):
    pressure_loss = calculate_pressure_loss(eta0, b, melt_temp, nozzle_diameter / 1000, injection_speed, shot_weight, gate_diameter / 1000, nozzle_length / 1000, mold_temp, cycle_time)
